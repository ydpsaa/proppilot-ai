...
function useNewsEvents() {
  ...
  useEffect(() => {
    ...
    if (cached && (Date.now() - cached.ts) < 3_600_000) {
      setEvents(cached.data); setLoading(false); return;
    }
    fetch('https://nfs.faireconomy.media/ff_calendar_thisweek.json?version=9')
      .then(r => { if (!r.ok) throw new Error('fetch'); return r.json(); })
      .then(data => {
        if (!Array.isArray(data)) throw new Error('bad data');
        const todayISO = new Date().toISOString().slice(0, 10); // "2026-04-26"
        const todayEvts = data.filter(e => {
          if (!e.date) return false;
          return e.date.startsWith(todayISO);
      ...
      .catch(() => {
        setEvents([{
          impact:'High', title:'CPI / NFP — check ForexFactory',
          country:'USD', time:'Check calendar', date: new Date().toISOString().slice(0,10),
        }]);
      })
